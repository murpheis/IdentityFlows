...
quietly infix                   ///
  ...
  byte    msacmsz      55-56    ///
  long    metfips      57-61    ///
  byte    individcc    62-62    ///
  double  marbasecidh  63-72    ///
  byte    pernum       73-74    ///
  double  wtfinl       75-88    ///
  double  cpsidp       89-102   ///
  int     relate       103-106  ///
  byte    age          107-108  ///
  byte    sex          109-109  ///
  int     race         110-112  ///
  byte    marst        113-113  ///
  byte    popstat      114-114  ///
  ...
  int     ind          135-138  ///
  int     ind1950      139-141  ///
  byte    classwkr     142-143  ///
  int     uhrsworkt    144-146  ///
  int     uhrswork1    147-149  ///
  int     uhrswork2    150-152  ///
  int     ahrsworkt    153-155  ///
  int     ahrswork1    156-158  ///
  int     ahrswork2    159-161  ///
  byte    absent       162-162  ///
  byte    durunem2     163-164  ///
  int     durunemp     165-167  ///
  byte    whyunemp     168-168  ///
  byte    wkstat       169-170  ///
  byte    profcert     171-172  ///
  byte    jobcert      173-174  ///
  int     educ         175-177  ///
  byte    educ99       178-179  ///
  double  marbasecidp  180-189  ///
  using `"/scratch/public/murpheis/CPS/input/cps_00003.dat"'
...
saveold "/scratch/public/murpheis/CPS/output/CPS.dta", replace


* KEEP 1% SUBSET OF DATA THAT I CAN LOOK AT ON MY OWN COMP
bysort cpsidp: gen rand = runiform()
keep if rand <= 0.05
drop rand
bysort cpsidp: gen N = _N
capture tab N
saveold "/scratch/public/murpheis/CPS/output/CPS_5pct.dta", replace
/*
...
keep year month region wtfinl cpsidp age sex race marst empstat occ1990 ///
     ind1990 durunemp wkstat educ classwkr labforce broadInd
bysort cpsidp (year month): gen n = _n
bysort cpsidp (year month): gen N = _N
drop if N > 8
drop N
reshape wide year month region wtfinl age sex race marst empstat occ1990 ///
        ind1990 durunemp wkstat educ classwkr labforce broadInd, i(cpsidp) j(n)

* OUTPUT AS STATA FILE
saveold "/scratch/public/murpheis/CPS/output/CPSwide.dta", replace
...
keep if rand <= 0.01
drop rand
saveold "/scratch/public/murpheis/CPS/output/CPSwide_1pct.dta", replace
...
label var msacmsz     `"Metropolitan area size (CMSA/MSA)"'
...
label var profcert    `"Has active professional certification or license"'
label var jobcert     `"Certification is required for job"'
...
label define msacmsz_lbl 00 `"Not identified or nonmetropolitan"'
label define msacmsz_lbl 01 `"100,000 - 249,999"', add
label define msacmsz_lbl 02 `"250,000 - 499,999"', add
label define msacmsz_lbl 03 `"500,000 - 999,999"', add
label define msacmsz_lbl 04 `"1,000,000 - 2,499,999"', add
label define msacmsz_lbl 05 `"2,500,000 - 4,999,999"', add
label define msacmsz_lbl 06 `"5,000,000 or more"', add
label values msacmsz msacmsz_lbl
...
label define profcert_lbl 01 `"No"'
label define profcert_lbl 02 `"Yes"', add
label define profcert_lbl 99 `"NIU"', add
label values profcert profcert_lbl

label define jobcert_lbl 01 `"No"'
label define jobcert_lbl 02 `"Yes"', add
label define jobcert_lbl 99 `"NIU"', add
label values jobcert jobcert_lbl
